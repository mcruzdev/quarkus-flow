...
The `regular` strategy preserves the exact workflow name as defined in your YAML, while the `class` strategy converts the name to PascalCase for a Java class-like identifier.
...
----
<1> Use the `@Identifier` annotation with the identifier derived from your YAML file. With the default `regular` naming strategy, this follows the `namespace:name` format.
