...
Quarkus Flow can create the persistence tables automatically through the Quarkus Hibernate ORM link:https://quarkus.io/guides/hibernate-orm#quarkus-hibernate-orm_quarkus-hibernate-orm-database-generation[schema management configuration]:
...
This is convenient for development and testing. For production, prefer managing the schema explicitly with a migration tool such as link:https://quarkus.io/guides/flyway[Flyway].
...
[NOTE]
====
The script above uses H2 syntax. Adapt the column types to your target database — for example, on PostgreSQL use `BYTEA` instead of `VARBINARY` and `SMALLINT` instead of `TINYINT`.
====
